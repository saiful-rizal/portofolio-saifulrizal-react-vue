...
<template>
  <div class="profile-page">
    <div class="profile-card">
      ...
      <p class="x-eyebrow">REKAM JEJAK</p>
      <h1 class="x-title">Achievement</h1>
      <p class="x-sub">Penghargaan, pendanaan, dan kompetisi yang memvalidasi kualitas kerja saya.</p>
      ...
      <div class="ac-timeline">
        <div v-for="a in items" :key="a.title" class="ac-item">
          <span class="ac-dot" />
          <div class="ac-card">
            <div class="ac-top">
    ...
    <nav class="p-nav" aria-label="Navigasi utama">
      ...
      <button class="p-nav-item wide" :class="{ active: active === 'achievement' }" @click="go('achievement')">
...
.ac-timeline{display:flex;flex-direction:column;gap:0;text-align:left;position:relative;margin-left:8px;padding-left:22px;border-left:1px solid rgba(255,255,255,.18)}
.ac-item{position:relative;padding:0 0 14px}
.ac-dot{position:absolute;left:-28px;top:14px;width:11px;height:11px;border-radius:50%;background:#fff;border:3px solid #333}
.ac-card{background:#0d0d0d;border:1px solid rgba(255,255,255,.14);border-radius:12px;padding:13px 14px;transition:border-color .2s}
...
.ac-year{font-size:10px;font-weight:800;color:#111;background:#fff;border-radius:5px;padding:2px 9px}
.ac-tag{font-size:9px;color:#bbb;border:1px solid rgba(255,255,255,.25);border-radius:999px;padding:2px 9px}
.ac-card h3{font-size:12.5px;font-weight:800;color:#fff;margin:0 0 2px}
.ac-org{font-size:10.5px;color:#cfcfcf;margin:0 0 6px}
.ac-desc{font-size:10.5px;line-height:1.65;color:#9a9a9a;margin:0}
